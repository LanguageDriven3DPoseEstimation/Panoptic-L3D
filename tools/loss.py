...
import torch
from tools.det_utils import _tranpose_and_gather_feat
...
class PerJointMSELoss(nn.Module):
    ...
    def forward(self, output, target, use_target_weight = False, target_weight=None):
        if use_target_weight:
            ...
            loss1 = self.criterion(heatmap_pred.mul(target_weight), heatmap_gt.mul(target_weight))
            mse = torch.square(torch.sub(heatmap_pred, heatmap_gt))
            num_k = int(heatmap_pred.shape[2] * 0.01)
            mse_topk = torch.topk(mse, num_k, dim=2, largest=True)[0]
            loss = mse_topk.mean() + loss1
        else:
...
class RegL1Loss(nn.Module):
    ...
    def forward(self, output, mask, ind, target, person_idx):
        pred = _tranpose_and_gather_feat(output, ind.gather(1,person_idx))
        target = target.squeeze(2)
        pred = pred.squeeze(2)
        loss = F.l1_loss(pred, target.gather(1,person_idx), size_average=False)
        return loss


class RegL1Loss_ori(nn.Module):
    def __init__(self):
        super(RegL1Loss_ori, self).__init__()

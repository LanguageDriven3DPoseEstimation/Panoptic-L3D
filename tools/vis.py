...
def save_debug_images_multi(config, input, meta, target, output, pred, prefix):
    ...
    if config.VIS.SAVE_BATCH_IMAGES_GT:
        save_batch_image_with_joints_multi(input, meta['joints'], meta['joints_vis'], meta['num_person'], '{}_gt.jpg'.format(prefix1))
    if config.VIS.SAVE_BATCH_IMAGES_PRED:
        pose2d = torch.zeros_like(meta['joints'][:,meta['person_idx'].squeeze()].unsqueeze(1)).cuda()
        if isinstance(pred, torch.Tensor):
            pred_torch = pred.clone()
        else:
            pred_torch = torch.tensor(pred.copy()).cuda()
        joints_vis = torch.ones_like(meta['joints_vis'][:,meta['person_idx'].squeeze()].unsqueeze(1))
        ...
        for i in range(pose2d.shape[0]):
            ...
            trans = torch.as_tensor(
                get_transform(center, scale, 0, config.DATA.IMAGE_SIZE),
                dtype=torch.float,
...
def save_debug_3d_images(config, meta, preds, prefix):
    ...
    for i in range(batch_size):
        ...
        person_idx = meta['person_idx'].squeeze()
        # joints_3d = meta['joints_3d'][i][meta['person_idx'].squeeze()].unsqueeze(0)
        # joints_3d_vis = meta['joints_3d_vis'][i][meta['person_idx'].squeeze()].unsqueeze(0)
            ...
            for k in eval("LIMBS{}".format(len(joint))):
                if joint_vis[k[0], 0] and joint_vis[k[1], 0]:
                    ...
                    if n == person_idx:
                        ...
                        ax.plot(x, y, z, c='b', lw=1.5, marker='o', markerfacecolor='w', markersize=2,
                                markeredgewidth=1,alpha=0.4)
                else:
                    x = [float(joint[k[0], 0]), float(joint[k[1], 0])]
                    y = [float(joint[k[0], 1]), float(joint[k[1], 1])]
                    ...
                    if n == person_idx:
                        ...
                        ax.plot(x, y, z, c='b', ls='--', lw=1.5, marker='o', markerfacecolor='w', markersize=2,
                                markeredgewidth=1,alpha=0.4)
            ...
            for n in range(len(pred)):
                ...
                if joint[0, 3] >= 0:
                    for k in eval("LIMBS{}".format(len(joint))):
                        ...
                        y = [float(joint[k[0], 1]), float(joint[k[1], 1])]
                        z = [float(joint[k[0], 2]), float(joint[k[1], 2])]
                        ax.plot(x, y, z, c='orange', lw=1.5, marker='o', markerfacecolor='w', markersize=2,
                                markeredgewidth=1)
    plt.savefig(file_name)
    plt.close(0)

def save_debug_3d_gt_images(config, meta, preds, prefix):
    basename = os.path.basename(prefix)
    dirname = os.path.dirname(prefix)
    dirname1 = os.path.join(dirname, '3d_joints')
...
def save_debug_mask(meta, image_batch, gt_mask, pred_mask, prefix, alpha=0.6, normalize=True):

    colormap = [0, 0, 0, 0, 0, 128, 0, 80, 128]
    colormap = np.array(colormap).reshape((-1,3))

    basename = os.path.basename(prefix)
    dirname = os.path.dirname(prefix)
    dirname1 = os.path.join(dirname, 'mask_imgs')
    ...
    for i in range(len(img_paths)):
        img = image_batch[i].permute(1, 2, 0).cpu().numpy()
        cv2.imwrite(prefix + '_origin_img.png', img)

        pred_mask_img = img.copy()
        color = np.array(colormap[2])
        pred_mask_idx = pred_mask.cpu().squeeze().numpy() > 0.5
        pred_mask_img[pred_mask_idx] = pred_mask_img[pred_mask_idx] * (1 - alpha) + color * alpha
        cv2.imwrite(prefix + '_mask_img_pred.png', pred_mask_img)

        with open((prefix+'_sentences.txt'), 'w') as file:
            file.write(meta['sentences'][i])

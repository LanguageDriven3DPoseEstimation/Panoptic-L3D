...
def det_decode(heat, wh, depth, K=100):
    ...
    # assert 0
    if depth is None:
        return bboxes, None, scores, inds, torch.cat((xs, ys), dim=-1)
    depth = _tranpose_and_gather_feat(depth, inds).clamp(min=0.)  # 同理
    depth = depth.view(batch, K, 1)
    depth[scores < 0.1] = -1
    sorted_indices = torch.argsort(depth, dim=1, descending=True)

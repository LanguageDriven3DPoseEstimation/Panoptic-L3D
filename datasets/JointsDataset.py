...
from tools.transforms import affine_transform, get_scale
from tools.cameras_cpu import project_pose, world_to_camera_frame, angles_to_camera
...
class JointsDataset(Dataset):
    ...
    def __init__(self, cfg, image_set):
        ...
        # dataset_root =
        self.dataset_root = os.path.join(cfg.DATA.ROOT, 'panoptic-L3D')
        self.root_id = cfg.DATA.ROOTIDX
        self.image_set = image_set
        self.dataset_name = cfg.DATA.NAME

        self.data_format = cfg.DATA.DATA_FORMAT
        self.data_augmentation = cfg.DATA.DATA_AUGMENTATION
        self.heatmap_noise = cfg.DATA.HEATMAP.NOISE
        self.heatmap_sigma_scale = cfg.DATA.HEATMAP.SIGMA_SCALE
        self.heatmap_sigma = cfg.DATA.HEATMAP.SIGMA
        self.fix_camera = cfg.DATA.HEATMAP.FIX_CAMERA
        self.random_place = cfg.DATA.RANDOM_PLACE
        self.random_split_ratio = cfg.DATA.RANDOM_SPLIT_RATIO
        self.window_num = cfg.MODEL.WINDOW_NUM

        self.scale_factor = cfg.DATA.SCALE_FACTOR
        self.scale_factor_range = cfg.DATA.SCALE_FACTOR_RANGE
        self.rotation_factor = cfg.DATA.ROT_FACTOR
        self.color_rgb = cfg.DATA.COLOR_RGB

        self.target_type = cfg.MODEL.TARGET_TYPE
        self.image_size = np.array(cfg.DATA.IMAGE_SIZE)
        self.heatmap_size = np.array(cfg.DATA.HEATMAP_SIZE)
        ...
        self.bbox_extention = cfg.DATA.BBOX_EXTENTION
    ...
    def __getitem__(self, idx):
        ...
        target_heatmap, target_weight = self.generate_target_heatmap(
            joints, joints_vis)
        # target_heatmap_one, target_weight_one = self.generate_target_heatmap_one(joints,joints_vis,db_rec['body_id'])
        ...
        return input, target_heatmap, target_weight, target_3d, meta, input_AGR, input, input, input, input, input
    ...
    def generate_target_heatmap_one(self, joints, joints_vis, id):
        num_joints = self.num_joints
        target_weight = np.zeros((num_joints, 1), dtype=np.float32)

        try:
            for i in range(num_joints):
                    if joints_vis[id][i, 0] == 1:
                        target_weight[i, 0] = 1
        except:
            assert 0, (joints.shape, joints_vis.shape, id, joints_vis[id][0, 0], target_weight.shape)
        ...
        if self.target_type == 'gaussian':
            ...
            # human_scale = 2 * self.compute_human_scale(joints[n] / feat_stride, np.ones_like(joints_vis[n]))
            human_scale = 2 * self.compute_human_scale(joints[id] / feat_stride, joints_vis[id])

            if self.heatmap_sigma_scale:
                cur_sigma = self.heatmap_sigma * np.sqrt((human_scale / (96.0 * 96.0)))
            else:
                cur_sigma = self.heatmap_sigma * np.sqrt(
                    (self.heatmap_size[0] * self.heatmap_size[1] / (96.0 * 96.0)))
            tmp_size = cur_sigma * 3
            for joint_id in range(num_joints):
                feat_stride = self.image_size / self.heatmap_size
                mu_x = int(joints[id][joint_id][0] / feat_stride[0])
                mu_y = int(joints[id][joint_id][1] / feat_stride[1])
                ul = [int(mu_x - tmp_size), int(mu_y - tmp_size)]
                br = [int(mu_x + tmp_size + 1), int(mu_y + tmp_size + 1)]
                if (joints_vis[id][joint_id, 0] == 0) or \
                        ul[0] >= self.heatmap_size[0] or \
                        ul[1] >= self.heatmap_size[1] \
                        or br[0] < 0 or br[1] < 0:
                    continue
    ...
    def generate_3d_target_one(self, joints_3d, id):
        ...
        joint_id = self.root_id  # mid-hip
        if isinstance(joint_id, int):
            mu_x = joints_3d[id][joint_id][0]
            mu_y = joints_3d[id][joint_id][1]
            mu_z = joints_3d[id][joint_id][2]
        elif isinstance(joint_id, list):
            mu_x = (joints_3d[id][joint_id[0]][0] + joints_3d[id][joint_id[1]][0]) / 2.0
            mu_y = (joints_3d[id][joint_id[0]][1] + joints_3d[id][joint_id[1]][1]) / 2.0
            mu_z = (joints_3d[id][joint_id[0]][2] + joints_3d[id][joint_id[1]][2]) / 2.0
        i_x = [np.searchsorted(grid1Dx,  mu_x - 3 * cur_sigma),
                   np.searchsorted(grid1Dx,  mu_x + 3 * cur_sigma, 'right')]
        i_y = [np.searchsorted(grid1Dy,  mu_y - 3 * cur_sigma),
                   np.searchsorted(grid1Dy,  mu_y + 3 * cur_sigma, 'right')]
        i_z = [np.searchsorted(grid1Dz,  mu_z - 3 * cur_sigma),
                   np.searchsorted(grid1Dz,  mu_z + 3 * cur_sigma, 'right')]
        if i_x[0] < i_x[1] and i_y[0] < i_y[1] and i_z[0] < i_z[1]:
            gridx, gridy, gridz = np.meshgrid(grid1Dx[i_x[0]:i_x[1]], grid1Dy[i_y[0]:i_y[1]], grid1Dz[i_z[0]:i_z[1]], indexing='ij')
            g = np.exp(-((gridx - mu_x) ** 2 + (gridy - mu_y) ** 2 + (gridz - mu_z) ** 2) / (2 * cur_sigma ** 2))
            target[i_x[0]:i_x[1], i_y[0]:i_y[1], i_z[0]:i_z[1]] = np.maximum(target[i_x[0]:i_x[1], i_y[0]:i_y[1], i_z[0]:i_z[1]], g)
    ...
    def generate_input_heatmap(self, joints, joints_cam, heatmap_size=None):
        ...
        '''
        nposes = len(joints)
        num_joints = self.cfg.DATA.NUM_JOINTS
        ...
        if self.target_type == 'gaussian':
            ...
            for n in range(nposes):
                human_scale = 2 * self.compute_human_scale(joints[n][:, 0:2] / feat_stride, np.ones((num_joints, 1))) # 5799
                if human_scale == 0:
                    continue
